...
async function resetPasswordEmail(currentInput) {
    ...
    const apiUrl = "https://api.payuee.com/web/forgotten-password-email";
    ...
    try {
        ...
        if (!response.ok) {
            ...
            } else if (data.error == 'failed to find user by email') {
                showError('emailError', 'Sorry user do not exist with us.');
            }else {
